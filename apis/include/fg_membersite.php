...
class FGMembersite
{
    ...
    function SendUserWelcomeEmail(&$user_rec)
    {
        ...
        " \r\n ".        
        "  You can access this key on the website <a href='http://rushabhgosar.com/apis/'>here</a>, after logging in." .
        " Regards,\r\n ".
        " Webmaster\r\n ".
        $this->sitename;
    ...
    function InsertIntoDB(&$formvars)
    {
        ...
        $confirmcode = $this->MakeConfirmationMd5($formvars['email'] + md5($formvars['password']));
   ...
   function EnsureScoretable()
    {
        ...
        {
            $invalidAPI = array("Response" => "Invalid API Key! Ensure");
            echo json_encode ($invalidAPI);
            return;
        }
...
function RetrieveScores($apikey)
{
    $this->scoreTableName .= $apikey;   
    $this->DBLogin();
    if(!$this->EnsureScoretable())
    ...
    if(!$this->APIexists($apikey))
    {
        echo json_encode($invalidAPI = array("Response" => "Invalid API Key!"));
        return;
    }
    $sql = 'Select * from ' . $this->scoreTableName;
    $userinfo = array();
    $res = mysql_unbuffered_query($sql);
    if(!$res)
    {
        echo json_encode(array("Response" => "An internal error occured"));
        echo mysql_error();
        return;        
    }
    ...
    $xml_op = $this->array_to_xml($userinfo, $xml_student_info);
    //echo $xml_op;    
